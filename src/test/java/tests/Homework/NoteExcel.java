...
We don't want to store test data in variables. Instead, we want to be able to change test data
 without affecting test script. When application requires testing with multiple data inputs we
 can use Data Driven Framework.
...
In TestNG we can run same test multiple times with @DataProvider. IF we combine data provider
with excel file, then we can run same test multiple times with data that is coming from excel file.
