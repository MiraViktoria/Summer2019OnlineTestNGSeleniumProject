...
testng xml runners helps to develop test suits. We can specify what tests we want to include
into specific suit. For example for major regression, we need to include all tests that we have.
But, when it comes to minor regression, we need to specify some group of tests only.
...
[TestNGContentHandler] [WARN] It is strongly recommended to add
 "<!DOCTYPE suite SYSTEM "http://testng.org/testng-1.0.dtd" >" at the top of your file,
  otherwise TestNG may fail or not work as expected. ---------->>> what does this line means?
