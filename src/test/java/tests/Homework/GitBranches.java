...
Excel file is very popular way to store data. It can contains thousands of rows
and columns with data. In order to retrieve and inject that data with java we use Apache POI library.
...
DDT Data-driven testing is a software testing methodology. The idea is to retrieve test data from outside.
 We don't want to store test data in variables. Instead, we want to be able to change test data without
  affecting test script. When application requires testing with multiple data inputs we can use Data Driven Framework.
